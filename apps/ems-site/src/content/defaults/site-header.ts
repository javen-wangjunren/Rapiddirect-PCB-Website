import type { SiteHeaderData } from '../schemas/site-header';
...
const emptyPlatform = {
  platform_cards: [] as { image_url: string; title: string; description: string; links: { label: string; href: string }[]; list_style: 'simple' | 'timeline'; cta_label: string; cta_href: string }[]
};

const emptyResources = {
  resource_sections: [] as { section_title: string; link_style: 'simple' | 'service'; links: { label: string; href: string }[]; service_items: { title: string; desc: string; href: string }[]; footer_label: string; footer_href: string }[]
};
...
const emptyAbout = {
  ...
  about_banner_title: '',
  about_banner_desc: '',
  about_link_groups: [] as { group_title: string; links: { label: string; href: string }[] }[]
};

export const siteHeaderDefaults: SiteHeaderData = {
  logo_url: '',
      ...
      sections: [
        {
          ...
          tabs: [
            {
              tab_label: 'Machining', is_muted: false,
              cards: [
                { label: 'CNC Machining', href: '#' }, { label: 'CNC Milling', href: '#' },
                { label: 'CNC Turning', href: '#' }, { label: 'CNC Routing', href: '#' },
                { label: '5 Axis CNC Machining', href: '#' }, { label: 'Precision CNC', href: '#' }
              ], image_url: ''
            },
            {
              tab_label: 'Fabrication', is_muted: false,
              cards: [
                { label: 'Sheet Metal Fabrication', href: '#' }, { label: 'Laser Cutting', href: '#' },
                { label: 'Metal Bending', href: '#' }, { label: 'Waterjet Cutting', href: '#' },
                { label: 'Tube Laser Cutting', href: '#' }, { label: 'Custom Enclosure', href: '#' },
                { label: 'Welding Services', href: '#' }
              ], image_url: ''
            },
            {
              tab_label: 'Molding', is_muted: false,
              cards: [
                { label: 'Injection Molding', href: '#' }, { label: 'Injection Mold Tooling', href: '#' },
                { label: 'Overmolding', href: '#' }, { label: 'Insert Molding', href: '#' },
                { label: 'Low Volume Injection Molding', href: '#' }
              ], image_url: ''
            },
            {
              tab_label: '3D Printing', is_muted: false,
              cards: [
                { label: '3D Printing Prototyping', href: '#' }, { label: 'SLA', href: '#' },
                { label: 'SLS', href: '#' }, { label: 'SLM', href: '#' },
                { label: 'FDM', href: '#' }, { label: 'MJF', href: '#' },
                { label: 'DLP', href: '#' }, { label: 'FGF', href: '#' }
              ], image_url: ''
            },
            {
              tab_label: 'Value-Added', is_muted: true,
              cards: [
                { label: '3D Printing', href: '#' }, { label: 'Die Casting', href: '#' },
                { label: 'Vacuum Casting', href: '#' }, { label: 'Wire EDM', href: '#' },
        ...
        },
        {
          ...
          tabs: [
            {
              tab_label: 'Core Services', is_muted: false,
              cards: [
                { label: 'EMS', href: '#' }, { label: 'PCB Design', href: '#' },
                { label: 'PCB Assembly', href: '#' }, { label: 'PCB Manufacturing', href: '#' },
    ...
    // ── 2. Solutions ──
    {
      ...
      tabs: [
        ...
        },
        {
          ...
          steps: [],
          cards: [
            { label: 'Rapid Prototyping', href: '#' }, { label: 'On Demand Manufacturing', href: '#' },
            { label: 'Surface Finishing', href: '#' }, { label: 'Assembly', href: '#' },
            { label: 'Industrial Automation', href: '#' }
          ],
          cta1_label: 'Browse manufacturing services', cta1_href: '#', cta2_href: '#', image_url: ''
        }
      ],
      ...emptyIndustries,
      ...emptyPlatform,
